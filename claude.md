...
- `explain-job {job url}` — replays the full filter/score pipeline for one job and explains its rating. Flags: `--profile` (alternate profile YAML), `--job-file` (supply job fields as YAML — required for LinkedIn/Indeed which can't be auto-fetched), `--force-score` (run the AI scorer even if a hard filter rejected the job), `--run-data` (path to a local run's job_results.json, default `job_results.json`), `--dump-job-file` (save the resolved job to YAML for reuse).
